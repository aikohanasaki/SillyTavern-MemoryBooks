...
- `/sideprompt "Relationship Tracker" {{macro}}="value"` - 執行自訂追蹤器
* 選擇 Side Prompt 後，斜線命令的自動完成會提示剩餘必填的執行階段巨集。
...
它們會在 lorebook 中建立獨立的 Side Prompt 項目，並與記憶建立同時執行。把它們想像成 **觀察您的故事並保持特定細節為最新狀態的助手**。
標準 ST 巨集（如 `{{user}}` 和 `{{char}}`）會在 `Prompt` 和 `Response Format` 中展開。非標準 `{{...}}` 巨集會變成手動執行所需的輸入。
...
4. 啟用您想要的範本 (稍後可以自訂)
5. 如果範本包含自訂執行階段巨集，就不會自動執行，必須使用 `/sideprompt` 手動執行。
...
* **自動或手動**：標準範本可以自動執行；包含自訂執行階段巨集的範本只能手動執行。
* **巨集支援**：`Prompt` 和 `Response Format` 會展開標準 ST 巨集，例如 `{{user}}` 和 `{{char}}`。
* **安全檢查**：如果範本包含自訂執行階段巨集，STMB 會在儲存/匯入時移除 `onInterval` 和 `onAfterMemory`，並顯示警告。
...
3. 寫下簡短、清晰的指令
   *(例如：「總是記錄每個場景的天氣狀況」)*
4. 如有需要，加入標準 ST 巨集或執行階段巨集，格式為 `{{macro}}="value"`。
5. 儲存並啟用它
6. 只有在啟用自動觸發器時，追蹤器才會隨時間更新此資訊
