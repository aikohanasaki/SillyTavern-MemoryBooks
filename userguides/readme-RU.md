...
**📘 [Руководство пользователя (RU)](userguide-RU.md)** |  **📋 [История версий и список изменений](../changelog.md)** | [Использование 📕 Memory Books с 📚 Lorebook Ordering](https://github.com/aikohanasaki/SillyTavern-LorebookOrdering/blob/main/guides/STMB%20and%20STLO%20-%20Russian.md)
> Синхронизировано с английским README: e9f1709 (2025-10-31)
...
- Локализованные промпты:
  - Английские промпты локализованы и возвращают воспоминания на ВАШЕМ языке.
  - Язык определяется по общим настройкам языка в ST.
  - Новые пользователи: никаких действий не требуется — STMB определит ваш язык и все сделает автоматически.
  - Текущие пользователи: чтобы переключиться на локализованные встроенные промпты, удалите `SillyTavern/data/(yourusername)/user/files/stmb-summary-prompts.json`, затем заново откройте менеджер промптов Summary. Файл будет создан заново с локализованными встроенными промптами. Примечание: сначала сделайте резервную копию, если вносили изменения!
...
- **Позиция:** ↑Char, ↓Cha, ↑EM, ↓EM, ↑AN, Outlet (и название поля).
