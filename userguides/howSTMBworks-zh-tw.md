...
側邊提示是輔助性的、由模板驅動的生成器，可將結構化筆記寫回您的知識庫（例如，追踪器、報告、演員表）。它們與“記憶生成”路徑分開，會建立獨立的側邊提示知識庫條目，並可依範本自動運行或按需運行。含有自訂執行階段巨集的範本只能手動運行。
...
3) 配置：
   ...
   - 提示：模型將遵循的指令文本。這裡會展開 ST 標準巨集。
   - 響應格式：附加到提示的可選指導塊（不是架構，只是說明）。這裡也會展開 ST 標準巨集。
   - 執行階段巨集：非標準 `{{...}}` 記號會成為 `/sideprompt` 的必填輸入，例如 `{{npc name}}="Jane Doe"`。
   - 觸發器：
...
使用 /sideprompt 手動運行
- 語法：/sideprompt "名稱" {{macro}}="value" [X‑Y]
  - 示例：
    • /sideprompt "狀態"
...
- 範本名稱必須加上引號，巨集值也必須加上引號。
- 在命令自動完成中選擇某個側邊提示後，STMB 會提示該範本仍缺少的必要巨集。

自動運行
- 記憶後：所有啟用了 onAfterMemory 觸發器的模板都使用已編譯的場景運行。STMB 以較小的並發限制批量運行，並可以顯示每個模板的成功/失敗信息。
- 含有自訂執行階段巨集的範本只能手動運行。STMB 在儲存/匯入這類範本時會移除 `onInterval` 和 `onAfterMemory`，並顯示警告。
...
- 如果範本包含自訂執行階段巨集，STMB 在儲存/匯入時會移除自動觸發器並顯示警告。
- 側邊提示需要將記憶知識庫綁定到聊天（或在手動模式下選擇）。如果未綁定，STMB 將顯示通知並跳過運行。

導入/導出和內置重置
- 導出：將您的側邊提示文檔另存為 JSON。
- 導入：以附加方式合併條目；重複項會被安全地重命名（不覆蓋）。如果匯入的範本包含自訂執行階段巨集，STMB 會自動移除 `onInterval` 和 `onAfterMemory` 並顯示警告。
...
- 運行時間
  ...
  - 側邊提示：可以在記憶後、按間隔閾值或使用 /sideprompt 手動運行。含有自訂執行階段巨集的範本只能手動執行。

- 提示形狀
  - 記憶路徑：使用具有嚴格 JSON 契約的專用“摘要提示管理器”預設；STMB 驗證/修復 JSON。
  - 側邊提示：使用模板的指令文本 + 可選的先前條目 + 可選的先前記憶 + 編譯的場景文本；不需要 JSON 架構（可選的響應格式僅為指導）。ST 標準巨集會在提示和響應格式中展開。

- 輸出和存儲
  - 記憶路徑：一個 JSON 對象：{ title, content, keywords } → 作為用於檢索的記憶條目存儲。
  - 側邊提示：純文本內容 → 作為標題為“名稱 (STMB SidePrompt)”的知識庫條目存儲（舊名稱會被識別以進行更新）。不需要關鍵字。非標準 `{{...}}` 記號是手動命令的必填輸入。
