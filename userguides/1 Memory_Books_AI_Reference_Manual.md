...
Pending records are saved using ST's settings API and verified by reading settings back, with a ten-second confirmation limit. A settings failure leaves the update in memory. Use Apply in the affected chat to apply and verify the marker, or Discard pending update to remove the pending record. There is no separate settings-save retry button. There is no browser recovery backup: refreshing before a successful settings save can lose an unpersisted update. If the chat save succeeded but settings cleanup failed, clicking Apply again verifies the already-saved marker and completes cleanup without generating another Memory. Failed marker verification must never be interpreted as successful completion.
