...
サイドプロンプトは、テンプレート駆動型の補助的なジェネレーターで、構造化されたメモをロアブックに書き戻します（例：トラッカー、レポート、キャストリスト）。これらは「メモリ生成」パスとは別のもので、独立したサイドプロンプト用ロアブックエントリを作成し、テンプレートに応じて自動またはオンデマンドで実行できます。カスタム実行時マクロを含むテンプレートは手動実行専用です。
...
管理場所
- サイドプロンプトマネージャー（STMB 内）を開き、テンプレートの表示、作成、インポート/エクスポート、有効化、または設定を行います。`{{user}}` や `{{char}}` のような ST 標準マクロは `Prompt` と `Response Format` で展開され、`{{...}}` の非標準マクロはランタイム入力として扱われます。
...
3) 設定：
   ...
   - プロンプト：モデルが従う指示テキスト。ST 標準マクロはここで展開されます。
   - 応答形式：プロンプトに追加されるオプションのガイダンスブロック（スキーマではなく、単なる指示です）。ここでも ST 標準マクロが展開されます。
   - ランタイムマクロ：非標準の `{{...}}` トークンは `/sideprompt` の必須入力になります。例：`{{npc name}}="Jane Doe"`
   - トリガー：
...
/sideprompt での手動実行
- 構文：/sideprompt "名前" {{macro}}="value" [X‑Y]
  - 例：
    • /sideprompt "ステータス"
...
- テンプレート名は引用符で囲む必要があり、マクロ値も引用符で囲む必要があります。
- スラッシュコマンドのオートコンプリートでテンプレートを選ぶと、そのテンプレートに必要な未入力マクロを STMB が提案します。
...
- カスタム実行時マクロを持つテンプレートは手動実行専用です。STMB はそのようなテンプレートを保存/インポートする際に `onInterval` と `onAfterMemory` を削除し、警告を表示します。
...
- テンプレートにカスタム実行時マクロが含まれている場合、STMB は保存/インポート時に自動トリガーを削除し、その旨を警告します。

インポート/エクスポートと組み込みのリセット
- エクスポート：サイドプロンプトドキュメントを JSON として保存します。
- インポート：エントリを追加的にマージします。重複は安全に名前が変更されます（上書きなし）。インポートしたテンプレートにカスタム実行時マクロがある場合、STMB は `onInterval` と `onAfterMemory` を自動的に削除し、警告を表示します。
...
- 実行タイミング
  ...
  - サイドプロンプト：メモリ後、間隔のしきい値、または /sideprompt で手動で実行できます。カスタム実行時マクロを持つテンプレートは手動のみです。

- プロンプトの形状
  - メモリパス：厳密な JSON 契約を持つ専用の「サマリープロンプトマネージャー」プリセットを使用し、STMB が JSON を検証/修復します。
  - サイドプロンプト：テンプレートの指示テキスト + オプションの前のエントリ + オプションの以前のメモリ + コンパイルされたシーンテキストを使用します。JSON スキーマは不要です（オプションの応答形式はガイダンスのみです）。ST 標準マクロは Prompt と Response Format で展開されます。

- 出力とストレージ
  - メモリパス：1 つの JSON オブジェクト：{ title, content, keywords } → 検索に使用されるメモリエンティティとして保存されます。
  - サイドプロンプト：プレーンテキストコンテンツ → 「名前（STMB SidePrompt）」というタイトルのロアブックエントリとして保存されます（レガシー名は更新のために認識されます）。キーワードは必須ではありません。非標準の `{{...}}` トークンは手動コマンドの必須入力です。
