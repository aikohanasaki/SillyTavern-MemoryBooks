...
- [빠른 시작](#-빠른-시작-첫-번째-기억-생성까지-5분)
- [STMB가 실제로 하는 일](#-stmb가-실제로-하는-일)
- [토큰 절약: 숨기기/다시 보이기](#-토큰-절약-숨기기다시-보이기)
- [요약 통합](#-아크arc-요약)
- [사이드 프롬프트](#-트래커사이드-프롬프트-템플릿-고급-기능)
- [정규식](#-regex-확장-프로그램을-이용한-고급-텍스트-제어)
- [설정](#-실제로-중요한-설정)
- [문제 해결](#-문제-해결-뭔가-잘-안될-때)
- [더 알아보기](#-st-memory-books-가-하지-않는-일)
...
* 제어 패널에서 **"Auto-create memory summaries(기억 요약 자동 생성)"** 항목을 찾으세요.
* 스위치를 **ON**으로 켭니다.
* **Auto-Summary Interval**을 **20-30 메시지**로 설정하세요 (처음 시작하기 좋은 값입니다).
* **Auto-Summary Buffer**는 처음엔 `0-2` 정도로 두는 것이 무난합니다.
...
* `/sideprompt "Relationship Tracker" {{macro}}="value"` - 커스텀 트래커 실행
* Side Prompt를 선택하면 슬래시 명령어 자동완성이 남은 필수 런타임 매크로를 제안합니다.
...
요약 통합은 오래된 STMB 기억과 요약을 더 큰 recap으로 묶어 긴 이야기를 관리하기 쉽게 만드는 기능입니다. 자세한 내용은 [readme.md](readme.md)를 참고하세요.

사이드 프롬프트는 다른 용도입니다. 사이드 프롬프트는 트래커 항목으로, 관계나 현재 목표 같은 "현재 상태"를 관리합니다. 요약 통합의 대상은 STMB 기억/요약입니다.
...
STMB는 AI에게 보내기 전과 저장하기 전에 선택한 Regex 스크립트를 실행할 수 있습니다. **Use regex (advanced)**를 켠 뒤 **📐 Configure regex…**를 열면, STMB 안에서 보내기 전과 저장하기 전 스크립트를 고를 수 있습니다.

**중요:** STMB에서 선택한 스크립트는 Regex 확장 프로그램에서 비활성화되어 있어도 실행될 수 있습니다.
