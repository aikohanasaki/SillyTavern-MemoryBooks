...
관리 위치
- 사이드 프롬프트 관리자 (STMB 내)를 열어 템플릿을 보거나, 만들거나, 가져오거나/내보내거나, 활성화하거나, 구성합니다. `{{user}}` 및 `{{char}}` 같은 ST 표준 매크로는 `Prompt`와 `응답 형식`에서 확장되며, 비표준 `{{...}}` 매크로는 런타임 입력으로 취급됩니다.
...
3) 구성:
   ...
   - 프롬프트: 모델이 따를 지침 텍스트. ST 표준 매크로가 여기서 확장됩니다.
   - 응답 형식: 프롬프트에 추가되는 선택적 안내 블록 (스키마가 아니라 지침일 뿐입니다). 여기서도 ST 표준 매크로가 확장됩니다.
   - 런타임 매크로: 비표준 `{{...}}` 토큰은 `/sideprompt`의 필수 입력이 됩니다. 예: `{{npc name}}="Jane Doe"`
   - 트리거:
...
/sideprompt로 수동 실행
- 구문: /sideprompt "이름" {{macro}}="value" [X‑Y]
  - 예:
    • /sideprompt "상태"
...
- 템플릿 이름은 따옴표로 묶어야 하고, 매크로 값도 따옴표로 묶어야 합니다.
- 명령 자동완성에서 템플릿을 선택하면 STMB가 해당 템플릿에 아직 필요한 매크로를 제안합니다.
...
- 사용자 지정 런타임 매크로가 있는 템플릿은 수동 실행만 가능합니다. STMB는 이러한 템플릿을 저장/가져오기할 때 `onInterval`과 `onAfterMemory`를 제거하고 경고를 표시합니다.
...
- 템플릿에 사용자 지정 런타임 매크로가 있으면 STMB는 저장/가져오기 시 자동 트리거를 제거하고 경고를 표시합니다.

가져오기/내보내기 및 내장 재설정
- 내보내기: 사이드 프롬프트 문서를 JSON으로 저장합니다.
- 가져오기: 항목을 추가적으로 병합합니다. 중복 항목은 안전하게 이름이 변경됩니다 (덮어쓰기 없음). 가져온 템플릿에 사용자 지정 런타임 매크로가 있으면 STMB는 `onInterval`과 `onAfterMemory`를 자동으로 제거하고 경고를 표시합니다.
...
- 실행 시기
  ...
  - 사이드 프롬프트: 메모리 후, 간격 임계값에서 또는 /sideprompt로 수동으로 실행할 수 있습니다. 사용자 지정 런타임 매크로가 있는 템플릿은 수동만 가능합니다.

- 프롬프트 모양
  - 메모리 경로: 엄격한 JSON 계약이 있는 전용 "요약 프롬프트 관리자" 사전 설정을 사용하고 STMB가 JSON을 확인/수리합니다.
  - 사이드 프롬프트: 템플릿의 지침 텍스트 + 선택적 이전 항목 + 선택적 이전 메모리 + 컴파일된 장면 텍스트를 사용합니다. JSON 스키마가 필요하지 않습니다 (선택적 응답 형식은 안내일 뿐입니다). ST 표준 매크로는 Prompt와 응답 형식에서 확장됩니다.

- 출력 및 저장
  - 메모리 경로: 하나의 JSON 객체: { title, content, keywords } → 검색에 사용되는 메모리 항목으로 저장됩니다.
  - 사이드 프롬프트: 일반 텍스트 내용 → "이름 (STMB SidePrompt)"이라는 제목의 로어북 항목으로 저장됩니다 (레거시 이름은 업데이트를 위해 인식됨). 키워드는 필요하지 않습니다. 비표준 `{{...}}` 토큰은 수동 명령의 필수 입력입니다.
