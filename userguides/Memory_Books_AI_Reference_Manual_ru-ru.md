...
> **При использовании расширения Presence:** Presence может позднее показать сообщения, скрытые STMB, поскольку оба расширения изменяют общий статус видимости сообщений SillyTavern. Инструкции по настройке см. в разделе [STMB и другие расширения](#23-stmb-и-другие-расширения).
...
Расширения SillyTavern работают параллельно и могут читать или изменять одни и те же данные SillyTavern. STMB не переопределяет и не отключает другие расширения и не имеет приоритета над ними. Когда действия расширений пересекаются, итоговый результат зависит от настроек и времени действия каждого задействованного расширения.

### 23.1 Общая видимость сообщений

Статус сообщения как скрытого является частью общего состояния сообщений SillyTavern. Это состояние не принадлежит исключительно STMB.

Настройки **Token Saving** в STMB могут скрывать обработанные сообщения после сохранения Memory. Другое расширение может позднее снова показать эти сообщения, и STMB не будет этому препятствовать. Аналогично, **Unhide hidden messages for memory generation** может показывать сообщения, пока STMB обрабатывает или повторно генерирует выбранный range.

### 23.2 Presence

Расширение Presence и STMB могут изменять скрытый или видимый статус chat messages. Если Presence показывает сообщения, скрытые STMB, настройка Token Saving в STMB не была удалена или проигнорирована: более позднее действие Presence изменило то же состояние сообщений SillyTavern.

Если вы используете Presence и хотите, чтобы скрытые STMB сообщения оставались скрытыми, используйте собственную функцию Presence для блокировки скрытых сообщений. В настоящее время Presence предоставляет для этого команду `/presenceLockHiddenMessages`. Запустите её для соответствующего диапазона сообщений и повторяйте по мере расширения этого диапазона. Актуальное поведение команды описано в документации Presence.

STMB не настраивает и не вызывает Presence автоматически, а управление участниками group chat в STMB не связано с Token Saving.
