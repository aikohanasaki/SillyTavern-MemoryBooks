...
3) 配置：
   ...
   - 提示：模型将遵循的指令文本。这里会展开 ST 标准宏。
   - 响应格式：附加到提示的可选指导块（不是架构，只是说明）。这里也会展开 ST 标准宏。
   - 运行时宏：非标准 `{{...}}` 记号会成为 `/sideprompt` 的必填输入，例如 `{{npc name}}="Jane Doe"`。
   - 触发器：
...
使用 /sideprompt 手动运行
- 语法：/sideprompt "名称" {{macro}}="value" [X‑Y]
  - 示例：
    • /sideprompt "状态"
...
- 模板名称必须用引号括起来，宏值也必须用引号括起来。
- 在命令自动补全中选择某个侧边提示后，STMB 会提示该模板仍缺少的必需宏。
...
- 含有自定义运行时宏的模板只能手动运行。STMB 在保存/导入这类模板时会移除 `onInterval` 和 `onAfterMemory`，并显示警告。
...
- 如果模板包含自定义运行时宏，STMB 在保存/导入时会移除自动触发器并显示警告。

导入/导出和内置重置
- 导出：将您的侧边提示文档另存为 JSON。
- 导入：以附加方式合并条目；重复项会被安全地重命名（不覆盖）。如果导入的模板包含自定义运行时宏，STMB 会自动移除 `onInterval` 和 `onAfterMemory` 并显示警告。
...
- 运行时间
  ...
  - 侧边提示：可以在记忆后、按间隔阈值或使用 /sideprompt 手动运行。含有自定义运行时宏的模板只能手动运行。

- 提示形状
  - 记忆路径：使用具有严格 JSON 契约的专用“摘要提示管理器”预设；STMB 验证/修复 JSON。
  - 侧边提示：使用模板的指令文本 + 可选的先前条目 + 可选的先前记忆 + 编译的场景文本；不需要 JSON 架构（可选的响应格式仅为指导）。ST 标准宏会在 Prompt 和响应格式中展开。

- 输出和存储
  - 记忆路径：一个 JSON 对象：{ title, content, keywords } → 作为用于检索的记忆条目存储。
  - 侧边提示：纯文本内容 → 作为标题为“名称 (STMB SidePrompt)”的知识库条目存储（旧名称会被识别以进行更新）。不需要关键字。非标准 `{{...}}` 记号是手动命令的必填输入。
